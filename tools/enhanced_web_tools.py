...
def get_tool_references(
    tool_name: Annotated[
        str, "Name of the tool to get references for (e.g., 'tavily', 'wikipedia')"
    ],
) -> str:
...
def search_arxiv(
    ...
    try:
        ...
        if "429" in error_msg or "Too Many Requests" in error_msg:
            return "arXiv API rate limit exceeded (HTTP 429). The arXiv tool is currently unavailable. Please do NOT try arXiv again right now. Instead, use alternative tools like 'search_wikipedia', 'enhanced_search_tavily', or 'search_github' to continue your research."
            
        return f"arXiv search failed. Error: {error_msg}"
